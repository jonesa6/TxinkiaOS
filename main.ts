    ...
    if (!disalbleclick) {
        ...
        if (mySprite.overlapsWith(mySprite2)) {
            ...
            if (game.ask("play advoid the rock")) {
                ...
                game.showLongText("this APP does not exist.", DialogLayout.Bottom)
                mouse()
                home()
...
//  + * - / numbers
function math(num: number, num2: number, num3: number): number {
    if (num3 == 1) {
        return num + num2
    } else if (num3 == 0) {
        return num * num2
...
function makecodearcadeos() {
    game.showLongText("welcome to the makecode arcade OS!", DialogLayout.Bottom)
    mouse()
    home()
}
...
let jokelist = ["what do you call a pile of bannas? a slipery slope.", "what did the cupcake say to the cupkake. you have not muffin yet.", "why did the cat cross the road. because the chichen had  a laser pointer."]
//  secure check
...
if (!blockSettings.exists("password")) {
    ...
    game.showLongText(" press B when it asks if you want to reset system then enter  3726 for the public-password to be able to enter your password. after you read this ", DialogLayout.Bottom)
    game.reset()
